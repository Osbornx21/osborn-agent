...
const (
	ProviderIDLLM               = "moonshot-llm"
	DefaultLLMBaseURL           = "https://api.moonshot.cn/v1"
	DefaultLLMModel             = "moonshot-v1-8k"
	SourceDocURLLLM             = "https://platform.kimi.com/docs/api/chat"
	SourceDocCheckedAt          = "2026-06-06"
	defaultMaxCompletionTokens  = 160
	defaultProviderErrorMessage = "provider request failed"
)

var (
	ErrMissingAPIKey = providers.NewProviderConfigurationError("moonshot api key is required")
...
func (p *LLM) newStreamRequest(ctx context.Context, req providers.LLMRequest) (*http.Request, error) {
	...
	if p.thinkingType != "" {
		body.Thinking = &moonshotThinking{Type: p.thinkingType}
	}
...
type moonshotChatMessage struct {
...
func decodeStreamChunk(payload string, toolCalls *providers.ToolCallDeltaAccumulator) (providers.LLMChunk, bool, error) {
	return providers.DecodeOpenAIStreamChunk(payload, toolCalls, "moonshot")
}
...
func decodeProviderError(resp *http.Response, apiKey string) error {
	var payload moonshotErrorPayload
	data, err := io.ReadAll(io.LimitReader(resp.Body, 4096))
	if err == nil && len(data) > 0 {
		_ = json.Unmarshal(data, &payload)
	}
	code := firstNonEmpty(payload.Error.Type, payload.Error.Code, http.StatusText(resp.StatusCode))
	message := firstNonEmpty(payload.Error.Message, defaultProviderErrorMessage)
...
func (e *ProviderError) Error() string {
	...
	return fmt.Sprintf("moonshot provider error: status=%d code=%s message=%s", e.StatusCode, e.Code, e.Message)
}
...
func sanitizeProviderMessage(message string, apiKey string) string {
	...
	if strings.Contains(message, "Bearer ") {
		return defaultProviderErrorMessage
	}
	return message
}

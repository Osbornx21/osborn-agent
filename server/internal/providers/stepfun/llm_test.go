...
func TestLLMMetadataMatchesOfficialDocs(t *testing.T) {
	...
	if provider.ModelID() != "step-3.7-flash" {
...
func TestLLMBuildsStepFunOpenAICompatibleStreamingRequest(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Authorization"); got != "Bearer fake-stepfun-key" {
		...
		if body["max_tokens"] != float64(160) {
			t.Fatalf("max_tokens = %v, want 160 for voice path", body["max_tokens"])
		}
		if body["reasoning_format"] != "deepseek-style" {
			t.Fatalf("reasoning_format = %v", body["reasoning_format"])
		}
	...
	provider := NewLLM(LLMOptions{
		...
		APIKey:          "fake-stepfun-key",
		Model:           "step-3.7-flash",
		ReasoningFormat: "deepseek-style",
		ReasoningEffort: "low",
...
func TestLLMParsesSSEFixturesAndSkipsReasoning(t *testing.T) {
	stream := strings.Join([]string{
		readFixture(t, "sse_first_chunk.sse"),
		readFixture(t, "sse_reasoning_delta_chunk.sse"),
...
func TestLLMMapsProviderErrorsWithoutLeakingSecret(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.Header.Get("Authorization"), "super-secret") {
			...
			_, _ = w.Write([]byte(`{"code":"AuthenticationError","message":"bad key: super-secret-stepfun-key"}`))
			return
		}
	...
	provider := NewLLM(LLMOptions{
		BaseURL: server.URL + "/v1",
		APIKey:  "super-secret-stepfun-key",
		Model:   "step-3.7-flash",
		Client:  server.Client(),
	})
	...
	if strings.Contains(err.Error(), "super-secret-stepfun-key") {
	...
	if providerErr.StatusCode != http.StatusUnauthorized || providerErr.Code != "AuthenticationError" {

...
func TestLLMMetadataMatchesOfficialDocs(t *testing.T) {
	...
	if provider.ModelID() != "Qwen/Qwen3.5-35B-A3B" {
...
func TestLLMBuildsOpenAICompatibleStreamingRequestWithVoiceSafeDefaults(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Authorization"); got != "Bearer sf-key" {
		...
		if body["model"] != "Qwen/Qwen3.5-35B-A3B" {
		...
		if body["enable_thinking"] != false {
			t.Fatalf("enable_thinking = %v, want false for voice fallback", body["enable_thinking"])
		}
...
func TestLLMMapsProviderErrorsWithoutLeakingSecret(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.Header.Get("Authorization"), "super-secret") {
			...
			_, _ = w.Write([]byte(`{"code":401,"message":"bad key: super-secret-siliconflow-key","data":null}`))
			return
		}
	...
	provider := NewLLM(LLMOptions{
		BaseURL: server.URL + "/v1",
		APIKey:  "super-secret-siliconflow-key",
		Model:   "Qwen/Qwen3.5-35B-A3B",
		Client:  server.Client(),
	})
	...
	if strings.Contains(err.Error(), "super-secret-siliconflow-key") {
	...
	if providerErr.StatusCode != http.StatusUnauthorized || providerErr.Code != "401" {

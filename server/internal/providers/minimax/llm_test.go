...
func TestLLMMetadataMatchesOfficialDocs(t *testing.T) {
	...
	if provider.ModelID() != "MiniMax-M3" {
...
func TestLLMParsesStreamingToolCallDeltasAfterArgumentsComplete(t *testing.T) {
	stream := strings.Join([]string{
		`data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"call-memory","type":"function","function":{"name":"memory_lookup","arguments":"{\"query\":\"低"}}]}}],"base_resp":{"status_code":0,"status_msg":"success"}}`,
		`data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"延迟\"}"}}]},"finish_reason":null}],"base_resp":{"status_code":0,"status_msg":"success"}}`,
		`data: {"choices":[{"delta":{},"finish_reason":"tool_calls"}],"base_resp":{"status_code":0,"status_msg":"success"}}`,
		`data: [DONE]`,
	}, "\n")
...
func TestLLMMapsProviderErrorsWithoutLeakingSecret(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if strings.Contains(r.Header.Get("Authorization"), "mm-secret") {
...
func TestLLMMissingAPIKeyIsProviderConfigurationError(t *testing.T) {
	provider := NewLLM(LLMOptions{
		Model: "MiniMax-M3",
	})
	...
	if !errors.Is(err, providers.ErrProviderConfiguration) {
		t.Fatalf("Stream() error = %v, want provider configuration error", err)
	}
}
...
func TestLLMContextCancelClosesStream(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"},\"finish_reason\":null}],\"base_resp\":{\"status_code\":0,\"status_msg\":\"success\"}}\n\n"))
		flusher.Flush()
		close(started)
		<-r.Context().Done()
	}))
